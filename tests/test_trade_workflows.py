...
class TradeWorkflowTests(BinderBridgeTestCase):
    ...
    def test_counter_offer_preloads_cards_and_links_trades(self):
        ...
        updated = app.row("SELECT * FROM trades WHERE id = ?", (trade_id,))

        self.assertEqual(counter_id, trade_id)
        self.assertEqual(updated["status"], "pending")
        self.assertEqual(updated["proposer_id"], bob_id)
        self.assertEqual(updated["recipient_id"], alice_id)
        self.assertIsNone(updated["counter_trade_id"])
        self.assertIsNone(updated["countered_from_trade_id"])
        counter_request = app.row("SELECT * FROM trade_items WHERE trade_id = ? AND side = 'requested'", (trade_id,))
        self.assertEqual(counter_request["card_name"], "Rhystic Study")
        alice = app.row("SELECT * FROM users WHERE id = ?", (alice_id,))
        detail_html = app.render_trade_detail(alice, trade_id)
        self.assertIn("Counter message", detail_html)
        self.assertIn("Rhystic Study", detail_html)
